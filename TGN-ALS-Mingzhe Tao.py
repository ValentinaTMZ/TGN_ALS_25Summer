...
def plvMatrix(eegData, window_size=256):
    """
    eegData: [long_time, num_electrodes] — all L or R trials for one subject
    Output: list of PLV matrices, one per window
    """
    ...
    for w in range(num_windows):
        ...
        if end > num_time_steps:  # skip incomplete final window
            break
        eeg_window = eegData[start:end, :]
        ...
        plv_list.append(plv_matrix)

    return plv_list


# create TemporalData as data object from each single EEG trial, with class label
def temporalData(plv_list, label):
    """
    plv_list: list of [num_electrodes x num_electrodes] PLV matrices (one per window)
    label: integer (0 or 1), the class label for this trial (e.g., 0 = 'L', 1 = 'R')
    
    Output: TemporalData object for one trial
    """
    src, dst, t, msg = [], [], [], []

    for w, plv in enumerate(plv_list):  # w = window index (used as timestamp)
    ...
    temporal_data = TemporalData(
        ...
        y=torch.tensor([label], dtype=torch.long)  # label for the entire trial
    )

    return temporal_data

# collect all trials of the same label 'L'(0) or 'R'(1) for one subject 
def aggregate_temporalData(subject_data, window_size=256):
    """
    ...
    for label, field in enumerate(['L', 'R']):  # 0 = L, 1 = R
        trials = subject_data[field][0]  # shape: [num_trials, time, 64]
        long_eeg = np.concatenate([i[:, :19] for i in trials], axis=0)  # [long_time, 19]

        plv_list = plvMatrix(long_eeg, window_size)
        if not plv_list:
            print(f"Skipping {field} — too short after concatenation.")
            continue

        #temp_data = temporalData(plv_list, label)
        #if temp_data.src.numel() == 0 or temp_data.dst.numel() == 0:
            #print(f"Skipping {field} — empty graph.")
            #continue
            
        for single_plv in plv_list:
            temp_data = temporalData([single_plv], label)  # 注意要传 list

            if temp_data.src.numel() == 0 or temp_data.dst.numel() == 0:
                print(f"Skipping window in {field} — empty graph.")
                continue
...
for subject_number in tqdm(subject_numbers, desc="Processing Subjects"):
    ...
    subject_temporal_data = aggregate_temporalData(subject_data, window_size=256) 
...
with open('TGN_ALSresults.json', 'w') as json_file:
